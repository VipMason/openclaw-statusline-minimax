...
class StreamingCardController {
    ...
    needsFooterMetrics() {
        ...
        return footer.tokens || footer.cache || footer.context || footer.model || footer.quota;
    }
    /** Fetch MiniMax quota info from mmx CLI */
    getQuotaMetrics() {
        try {
            const mmxPath = execSync('which mmx 2>/dev/null || echo /usr/bin/mmx', { encoding: 'utf8' }).trim().split('\n')[0];
            const raw = execSync(`${mmxPath} quota show 2>/dev/null`, { timeout: 5000 });
            const data = JSON.parse(raw.toString());
            const models = data.model_remains || [];
            const mstar = models.find(m => m.model_name === 'MiniMax-M*') || {};
            const codingVlm = models.find(m => m.model_name === 'coding-plan-vlm') || {};
            const codingSearch = models.find(m => m.model_name === 'coding-plan-search') || {};
            const mstarHourlyUsed = mstar.current_interval_usage_count || 0;
            const mstarHourlyTotal = mstar.current_interval_total_count || 1500;
            const mstarHourlyRemainsMs = mstar.remains_time || 0;
            const mstarHourlyWindowMs = mstar.end_time && mstar.start_time ? (mstar.end_time - mstar.start_time) : 5 * 3600 * 1000;
            const mstarWeeklyUsed = mstar.current_weekly_usage_count || 0;
            const mstarWeeklyTotal = mstar.current_weekly_total_count || 15000;
            const mstarWeeklyRemainsMs = mstar.weekly_remains_time || 0;
            const mstarWeeklyWindowMs = mstar.weekly_end_time && mstar.weekly_start_time ? (mstar.weekly_end_time - mstar.weekly_start_time) : 7 * 24 * 3600 * 1000;
            const codingVlmUsed = codingVlm.current_interval_usage_count || 0;
            const codingVlmTotal = codingVlm.current_interval_total_count || 150;
            const codingSearchUsed = codingSearch.current_interval_usage_count || 0;
            const codingSearchTotal = codingSearch.current_interval_total_count || 150;
            return {
                mstarHourlyUsed, mstarHourlyTotal, mstarHourlyRemainsMs, mstarHourlyWindowMs,
                mstarWeeklyUsed, mstarWeeklyTotal, mstarWeeklyRemainsMs, mstarWeeklyWindowMs,
                codingVlmUsed, codingVlmTotal,
                codingSearchUsed, codingSearchTotal,
            };
        } catch {
            return undefined;
        }
    ...
    async onError(err, info) {
        ...
        const sessionMetrics = this.needsFooterMetrics() ? await this.getFooterSessionMetrics() : undefined;
        const quotaMetrics = this.deps.resolvedFooter.quota ? this.getQuotaMetrics() : undefined;
        const footerMetrics = { ...sessionMetrics, ...quotaMetrics };
