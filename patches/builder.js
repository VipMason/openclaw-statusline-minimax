 ...
 */
function buildFooter(zhText, enText, isError, color) {
    const useColor = color && !isError;
    const zhContent = isError ? `<font color='red'>${zhText}</font>`
        : useColor ? `<font color='${color}'>${zhText}</font>` : zhText;
    const enContent = isError ? `<font color='red'>${enText}</font>`
        : useColor ? `<font color='${color}'>${enText}</font>` : enText;
    return [
        {
            tag: 'markdown',
            content: enContent,
...
function formatFooterRuntimeSegments(params) {
    ...
    if (footer?.cache && metrics) {
        ...
        if (read != null && inputVal != null) {
            const hit = read + inputVal > 0 ? Math.round((read / (read + inputVal)) * 100) : 0;
            detailZh.push(`缓存 ${hit}%`);
            detailEn.push(`Cache ${hit}%`);
        }
    }
    ...
    // --- Quota line: MiniMax API quota (mmx) ---
    if (footer?.quota && metrics) {
        const q = metrics;
        const quotaParts = [];
        if (typeof q.mstarHourlyRemainsMs === 'number' && typeof q.mstarHourlyWindowMs === 'number' && q.mstarHourlyWindowMs > 0) {
            const pct = q.mstarHourlyTotal > 0 ? Math.round((q.mstarHourlyUsed / q.mstarHourlyTotal) * 100) : 0;
            const bars = Math.round(pct / 10);
            const barStr = '▓'.repeat(bars) + '░'.repeat(10 - bars);
            const remains = formatRemains(q.mstarHourlyRemainsMs);
            const window = formatWindow(q.mstarHourlyWindowMs);
            const c = colorForPercent(pct);
            quotaParts.push(`<font color='${c}'>${barStr} ${pct}% ${remains}/${window}</font>`);
        }
        if (typeof q.mstarWeeklyRemainsMs === 'number' && typeof q.mstarWeeklyWindowMs === 'number' && q.mstarWeeklyWindowMs > 0) {
            const pct = q.mstarWeeklyTotal > 0 ? Math.round((q.mstarWeeklyUsed / q.mstarWeeklyTotal) * 100) : 0;
            const bars = Math.round(pct / 10);
            const barStr = '▓'.repeat(bars) + '░'.repeat(10 - bars);
            const remains = formatRemains(q.mstarWeeklyRemainsMs);
            const window = formatWindow(q.mstarWeeklyWindowMs);
            const c = colorForPercent(pct);
            quotaParts.push(`<font color='${c}'>${barStr} ${pct}% ${remains}/${window}</font>`);
        }
        if (typeof q.codingVlmUsed === 'number' && typeof q.codingVlmTotal === 'number' && q.codingVlmTotal > 0) {
            const pct = Math.round((q.codingVlmUsed / q.codingVlmTotal) * 100);
            const c = colorForPercent(pct);
            quotaParts.push(`<font color='${c}'>v ${q.codingVlmUsed}/${q.codingVlmTotal}</font>`);
        }
        if (typeof q.codingSearchUsed === 'number' && typeof q.codingSearchTotal === 'number' && q.codingSearchTotal > 0) {
            const pct = Math.round((q.codingSearchUsed / q.codingSearchTotal) * 100);
            const c = colorForPercent(pct);
            quotaParts.push(`<font color='${c}'>s ${q.codingSearchUsed}/${q.codingSearchTotal}</font>`);
        }
        if (quotaParts.length > 0) {
            detailZh.push(quotaParts.join(' | '));
            detailEn.push(quotaParts.join(' | '));
        }
    }
